    ...
    def load_from_snowflake_to_postgresql(snowflake_options: dict, pg_url: str, pg_properties: dict):
        ...
        try:
            ...
            with conn.cursor() as cursor:  # use the connection to truncate the existing table
                cursor.execute("TRUNCATE TABLE SHAMPOO_DATA RESTART IDENTITY CASCADE")
                conn.commit()
                print("SHAMPOO_DATA table truncated successfully.")
        except Exception as e:
            print(f"Failed to truncate table SHAMPOO_DATA: {e}")
        finally:
            if conn:
                conn.close()
        ...
        if SHAMPOO_df:
            SHAMPOO_df.write \
                .jdbc(url=jdbc_url, table="SHAMPOO_DATA", mode="overwrite", properties=pg_properties)
